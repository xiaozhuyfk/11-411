...
while(<HMM>) {
    if(($qqq, $qq, $q, $p) = (m/trans\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)/)) {
    $A{$qqq}{$qq}{$q} = log($p);
    $States{$qqq} = 1;
    $States{$qq} = 1;
    $States{$q} = 1;
...
while(<>) { # read in one sentence at a time
    ...
    foreach $q (keys %States) { # consider each possible current state
        foreach $qq (keys %States) { # each possible previous state
           foreach $qqq (keys %States) { # each possible previous state

            if(defined $A{$qqq}{$qq}{$q}  # only consider "non-zeros"
               and defined $B{$q}{$w[$i]}
               and defined $V{$i - 1}{$qqq}{$qq})
            {
                $v = $V{$i - 1}{$qqq}{$qq} + $A{$qqq}{$qq}{$q} + $B{$q}{$w[$i]};
                if(!(defined $V{$i}{$qq}{$q}) or $v > $V{$i}{$qq}{$q}) {
    ...
    # in the final state.
    $foundgoal = 0;
    foreach $qqq (keys %States) { # for each possible state for the last word
    foreach $qq (keys %States) { # for each possible state for the last word
    if(defined $A{$qqq}{$qq}{$final_state} and defined $V{$n}{$qqq}{$qq}) {
        $v = $V{$n}{$qqq}{$qq} + $A{$qqq}{$qq}{$final_state};
